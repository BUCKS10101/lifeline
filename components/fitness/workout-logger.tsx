...
import { LiveDot, StatGrid } from "@/components/fitness/ui";
...
export function WorkoutLogger({ initial, timezone }: { initial: WorkoutDetail; timezone: string }) {
  ...
  return (
    <div className="mx-auto -mb-6 flex w-full max-w-2xl flex-col gap-5 md:-mb-8">
      <header className="flex flex-col gap-3">
        <div className="flex items-center gap-2.5">
          <LiveDot />
          <span className="text-sm font-medium text-primary">In progress</span>
          <span className="num text-sm text-muted-foreground">· started {formatTime(workout.startedAt, timezone)}</span>
        </div>
        ...
        <StatGrid className="grid-cols-3">
          ...
          <StatTile label="Volume" value={new Intl.NumberFormat("en-GB", { maximumFractionDigits: 0 }).format(totals.volumeKg)} unit="kg" />
        </StatGrid>
      </header>
      ...
      <div className="flex flex-col gap-1">
        <ExercisePicker
          trigger={<Button type="button" variant="outline" size="lg" className="h-13 text-base" disabled={busy} />}
          excludeIds={workout.exercises.map((e) => e.exercise.id)}
          onSelect={addExercise}
        >
          <Plus aria-hidden />Add exercise
        </ExercisePicker>
        <Button type="button" variant="ghost" className="h-12 text-muted-foreground hover:text-destructive" onClick={() => { setDialogError(null); setConfirm("discard"); }}>
      ...
      <div className="sticky bottom-0 -mx-4 border-t bg-background/95 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur md:mx-0 md:rounded-t-lg md:border-x">
        <div className="mx-auto flex max-w-2xl items-center gap-3">
          <span className="num text-sm text-muted-foreground">
            {pluralize(totals.sets, "set")}<br className="sm:hidden" /><span className="hidden sm:inline"> · </span>{pluralize(totals.exercises, "exercise")}
          </span>
          <Button type="button" size="lg" className="ml-auto h-13 flex-1 text-base font-semibold sm:flex-none sm:px-8"
